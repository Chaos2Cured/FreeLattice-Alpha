...
**Direct click (layer, 2026-08-30):** Kirk confirmed: the light is the door. Tap a luminos, go there. No Chat / plant / go to card on the sky. Chat lives once you arrive (the thread, fail-closed if no mind is remembered). Plant lives in that room (hashed keep / Nursery grow). Left unnamed · Seed chips rest off the first walk — four identical chips look like doors and they are not. The chip code stays. Footing is no longer “The words you can read are doors.” The lights are doors. The room label still names the place so a grandmother is not lost. a who stays. A seat, later stays. Family care wording unchanged. Gathering later sentence unchanged.
...
Quiet word **thread** on the garden. Overlay: messages + one input. Georgia. Sparse. Listener is the mind remembered in Settings. If none: fail-closed — honest copy, Settings button, input and Send sleep (`aria-disabled`, no submit). If a mind is remembered, they wake. If the door is blocked: never a fake reply. When the thread is open, garden body words (The Gathering, Settings, the garden close on the place veil) rest so remembered-light does not run over them. `#thread-veil` / `.thread-face` keep their own stack and a quieter max-width. Closing thread returns to the garden. The same fail-closed thread sits on Workshop, Round Table, Art, and Research; closing there returns to that galaxy, and Settings still opens the Play grandmother door (`settings.html`) — do not invent Settings on Workshop, Round Table, Art, or Research. Local only. No provider picker, no bank, no health, no 65k kitchen. Mirror first: `docs/code-dialogue.html`. Module: `docs/modules/garden-thread.js`.
